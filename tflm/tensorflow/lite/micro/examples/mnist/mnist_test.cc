...
#include "tensorflow/lite/micro/examples/mnist/models/generated_mnist_float_model.h"
#include "tensorflow/lite/micro/examples/mnist/models/generated_mnist_int8_model.h"
#include "tensorflow/lite/micro/examples/mnist/models/generated_mnist_pruned_int8_model.h"
...
// added by i.jeong
// code for load mnist data
#include "tensorflow/lite/micro/examples/mnist/mnist_loader.h"

std::string x_test_path = "/workspace/tflm/tflite-micro/tensorflow/lite/micro/examples/mnist/data/t10k-images-idx3-ubyte";
std::string y_test_path = "/workspace/tflm/tflite-micro/tensorflow/lite/micro/examples/mnist/data/t10k-labels-idx1-ubyte";
...
using OpResolver = tflite::MicroMutableOpResolver<15>;
...
TfLiteStatus ProfileMemoryAndLatency() {
  ...
  tflite::RecordingMicroInterpreter interpreter(
      tflite::GetModel(tensorflow_lite_micro_examples_mnist_models_mnist_float_tflite), op_resolver, allocator,
...
TfLiteStatus LoadFloatModelAndPerformInference() {
  const tflite::Model* model =
      ::tflite::GetModel(tensorflow_lite_micro_examples_mnist_models_mnist_float_tflite);
...
TfLiteStatus LoadQuantModelAndPerformInference() {
  ...
  const tflite::Model* model =
      ::tflite::GetModel(tensorflow_lite_micro_examples_mnist_models_mnist_int8_tflite);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);
  ...
  for (int i = 0; i < data.height; i++){
    for (int j = 0; j < data.width; j++){
      ...
      float normalized = data.images[DATA_IDX][i * data.width + j] / 255.0f;
      int _quantized = static_cast<int8_t>(round(normalized / input_scale) + input_zero_point);
      // input->data.int8[i * data.width + j] = std::max(-128, std::min(127, _quantized));
      input->data.int8[i * data.width + j] = _quantized;
    }
  }
  TF_LITE_ENSURE_STATUS(interpreter.Invoke());
...
TfLiteStatus LoadQuantModelAndPerformInferenceForAllData() {
  ...
  for (uint16_t data_idx = 0; data_idx < NUM_SAMPLES; data_idx++){
    for (int i = 0; i < data.height; i++){
      for (int j = 0; j < data.width; j++){
        // int _quantized = round((data.images[data_idx][i * data.width + j] / 255.0) / input_scale) + input_zero_point;
        // input->data.int8[i * data.width + j] = std::max(-128, std::min(127, _quantized));
        float normalized = data.images[data_idx][i * data.width + j] / 255.0f;
        int _quantized = static_cast<int8_t>(round(normalized / input_scale) + input_zero_point);
        // input->data.int8[i * data.width + j] = std::max(-128, std::min(127, _quantized));
...
TfLiteStatus LoadPrunedQuantModelAndPerformInference() {
  // Map the model into a usable data structure. This doesn't involve any
  // copying or parsing, it's a very lightweight operation.
  const tflite::Model* model =
      ::tflite::GetModel(tensorflow_lite_micro_examples_mnist_models_mnist_pruned_int8_tflite);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);
  ...
  MicroPrintf("[pruned int8 model] y_pred: %d", std::distance(dequantized_output.begin(), std::max_element(dequantized_output.begin(), dequantized_output.end())));
  MicroPrintf("[pruned int8 model] y_real: %d\n", (int)data.labels[DATA_IDX]);
