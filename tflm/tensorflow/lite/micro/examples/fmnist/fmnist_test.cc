...
#include "tensorflow/lite/micro/examples/fmnist/models/generated_fmnist_pruned_int8_model.h"
...
TfLiteStatus ProfileMemoryAndLatency() {
  ...
  tflite::RecordingMicroInterpreter interpreter(
      tflite::GetModel(tensorflow_lite_micro_examples_fmnist_models_fmnist_float_tflite), op_resolver, allocator,
...
TfLiteStatus LoadFloatModelAndPerformInference() {
  const tflite::Model* model =
      ::tflite::GetModel(tensorflow_lite_micro_examples_fmnist_models_fmnist_float_tflite);
  ...
  for (int i = 0; i < data.height; i++){
    for (int j = 0; j < data.width; j++){
      ...
      // interpreter.input(0)->data.f[i * data.width + j] = ((data.images[DATA_IDX][i * data.width + j] / 255.0) - 0.5) / 0.5;
    }
  }
  TF_LITE_ENSURE_STATUS(interpreter.Invoke());
...
TfLiteStatus LoadQuantModelAndPerformInference() {
  ...
  const tflite::Model* model =
      ::tflite::GetModel(tensorflow_lite_micro_examples_fmnist_models_fmnist_int8_tflite);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);
  ...
  for (int i = 0; i < data.height; i++){
    for (int j = 0; j < data.width; j++){
      ...
      int _quantized = static_cast<int>(std::round(static_cast<float>(data.images[DATA_IDX][i * data.width + j]) / 255.0f / input_scale)) + input_zero_point;
...
TfLiteStatus LoadQuantModelAndPerformInferenceForAllData() {
  ...
  MicroPrintf("input_scale: %lf, input_zero_point: %d", (double)input_scale, input_zero_point);
...
TfLiteStatus LoadPrunedQuantModelAndPerformInference() {
  ...
  // copying or parsing, it's a very lightweight operation.
  const tflite::Model* model =
      ::tflite::GetModel(tensorflow_lite_micro_examples_fmnist_models_fmnist_pruned_int8_tflite);
  TFLITE_CHECK_EQ(model->version(), TFLITE_SCHEMA_VERSION);
...
TfLiteStatus LoadPrunedQuantModelAndPerformInferenceForAllData() {
  ...
  for (uint16_t data_idx = 0; data_idx < NUM_SAMPLES; data_idx++){
    ...
    std::vector<float> dequantized_output(NUM_CLASS);
    for (uint8_t i = 0; i < NUM_CLASS; i++) {
      // dequantized_output[i] = output->data.int8[i];
      dequantized_output[i] = (output->data.int8[i] - output_zero_point) * output_scale;
    }
    uint8_t y_pred = std::distance(dequantized_output.begin(), std::max_element(dequantized_output.begin(), dequantized_output.end()));
    if (y_pred == (int)data.labels[data_idx]){
